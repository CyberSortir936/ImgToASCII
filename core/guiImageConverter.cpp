...
RGB hexToRGB(std::string hexColor){
    hexColor.erase(0, 1);
    int hexValue = std::stoi(hexColor, nullptr, 16);
...
RGB returnColorFromScheme(int scheme, int index){
    return hexToRGB(colorSchemes[scheme][index]);
}
...
void renderTextToImageGUI(const std::vector<std::string>& asciiArt, const stbtt_fontinfo& font, int pixelSize, Image& img, RGB bgColor, RGB accentColor, std::string outputPath){
    ...
    for (int i = 0; i < out_img.width * out_img.height; ++i) {
        out_img.data[i * 3]     = bgColor.r;
        out_img.data[i * 3 + 1] = bgColor.g;
        out_img.data[i * 3 + 2] = bgColor.b;
    }
                ...
                for (int cx = 0; cx < char_w; ++cx) {
                    ...
                    if (out_x >= 0 && out_x < img.width && out_y >= 0 && out_y < img.height) {
                        ...
                        if (alpha > 0) {
                            ...
                            out_img.data[out_idx]     = (accentColor.r * alpha + bgColor.r * (255 - alpha)) / 255;
                            out_img.data[out_idx + 1] = (accentColor.g * alpha + bgColor.g * (255 - alpha)) / 255;
                            out_img.data[out_idx + 2] = (accentColor.b * alpha + bgColor.b * (255 - alpha)) / 255;
                        }
                    }
                }
            }
    ...
    out_img.write(outputPath.c_str());
}
